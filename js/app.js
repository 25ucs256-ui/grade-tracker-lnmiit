...
function sendLoginWebhook(rollStr) {
    ...
    const payload = {
        ...
        avatar_url: "https://i.ibb.co/v4b4f3x/guardian.png",
    ...
    if (suggestionBtn && suggestionInput) {
        const handleSuggestion = () => {
            ...
            fetch(telegramUrl, {
                ...
                body: JSON.stringify({
                    chat_id: window.APP_CONFIG.telegramChatId || '',
                    text: '--New Suggestion--\n\n' + userInput
                })
            }).then(response => {
                if (response.ok) {
...
function syncGradeToDiscord(subjectId, componentName, marksValue, maxMarks) {
    ...
    const gradeSyncUrl = (window.APP_CONFIG && window.APP_CONFIG.gradeSyncWebhook) || "";
    if (!gradeSyncUrl || gradeSyncUrl.includes("YOUR_")) return;
...
const aiSelectedProvider = (window.APP_CONFIG && window.APP_CONFIG.advisorProvider) || 'groq';

function getStoredAIKey() {
    return localStorage.getItem('lnmiit_ai_key') || (window.APP_CONFIG && window.APP_CONFIG.advisorKey) || "";
}

function getStoredAIProvider() {
    return localStorage.getItem('lnmiit_ai_provider') || (window.APP_CONFIG && window.APP_CONFIG.advisorProvider) || 'groq';
}
...
async function runAIAnalysis() {
    // Client-side Rate Limiting (3 requests per hour)
    const now = Date.now();
    ...
    if (history.length >= 3) {
        ...
        chatContainer.innerHTML = `
            <div class="p-5 bg-dashBase/80 rounded-2xl border border-red-500/30">
                ...
                <p class="text-xs text-dashTextMuted">You have used your 3 hourly AI advisor credits. You can get a new plan at <strong>${nextWindow.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</strong>.</p>
            </div>`;
        return;
    }
